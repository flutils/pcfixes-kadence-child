...
// --
// No need for a constructor because we are calling static methods (IE KadenceChild\Helpers::method())
class Helpers {

	// RPECK 17/08/2023 - Get global variable name
	// This was extracted from the Redux classes because we needed a standardized way to get it
	// --
	// Unfortunately, due to the variable nature of $opt_name, the function requires opt_name parameter
	static function getGlobalVariableName($opt_name) {

		// RPECK 11/08/2023 - Global Var
		...
		$global_variable_name = str_replace('-', '_', $opt_name);

		// RPECK 17/08/2023 - Return the global variable name
		// Allows us to use the value at will 
		return $global_variable_name;
		
	}

	// RPECK 17/08/2023 - Remove Emojis
	// Used to get rid of emoji's from the provided text (used primarily in the KadenceChild\Redux\Core class)
	// --
	// https://stackoverflow.com/a/69905879/1143732
	static function removeEmojis($text) {

		// RPECK 17/08/2023 - Return the text without emojis
		// This was taken directly from StackOverflow so may change over time
	...
	// RPECK 11/08/2023 - Check Colour Brightness
	// Added to give us the means to manage the font colours for the likes of admin menus (IE on darker colours, have light font + vice versa)
	// --
	// https://stackoverflow.com/questions/3015116/hex-code-brightness-php
	static function isBright($hex) {

    // RPECK 16/08/2023 - Remove hex value
    // Was raising deprecation errors with PHP 8.2+
    $hex = str_replace('#', '', $hex);
	...
	// RPECK 25/08/2023 - Repeater Data
	// Accepts an inbound block of repeater data values and returns an array formatted correctly
	// --
	// This is needed because of the strange way that Redux stores its repeater data
	static function repeaterData($value) {

		// RPECK 25/08/2023 - $val definition
		// Sets the declaration of $val so that we can populate it appropriately
		$val = array();

		// RPECK 25/08/2023 - Cycle through $value and get the various keys
		// Each array item will contain a main $key and sub $items
		foreach($value as $key => $v) {

			// RPECK 25/08/2023 - We need to cycle through $v first so that we are able to populate the $key appropriately
			// This is not efficient but works
			if(is_array($v) && count($v) > 0 && $key != 'redux_repeater_data') {

				// RPECK 25/08/2023 - Loop through V to get the appropriate values
				// This should then be used to populate $val 
				foreach($v as $index => $item) {

					// RPECK 25/08/2023 - Set $val[ $index ] as an array if it does not exist
					// Means that we can then populate it with other data or an array etc
					$val[ $index ] ??= array();

					// RPECK 25/08/2023 - Set item
					// We are able to use this to populate the actual array (which should end up as $val[0]['tag'] etc)
					$val[ $index ][ $key ] = $item;
		...
		// RPECK 25/08/2023 - Return
		// Sends back the compiled $val array with the appropriate data
		return $val;
		
	}

	// RPECK 27/08/2023 - Get Post Types
	// Allows us to pull in the various registered post types without rewriting code
	static function getPostTypes() {

		// RPECK 27/08/2023 - Vars
		// Used to provide the various properties used to pull in the post types
		$args 		= array('public' => true, '_builtin' => false);
		$output 	= 'names'; // names or objects, note names is the default
		$operator 	= 'and'; // 'and' or 'or'

		// RPECK 27/08/2023 - Get the post types
		// This is an inbuilt Wordpress function which allows us to pull in the various CPT's
		$post_types = get_post_types($args, $output, $operator); 

		// RPECK 27/08/2023 - Remove Kadence Elements
		// Kadence elements will be included with the CPT's, which we need to remove
		$post_types = array_filter( array_values($post_types) , function($post_type){
			return !str_starts_with($post_type, 'kadence_');
		});

		// RPECK 27/08/2023 - Return
		// Provides the values back to the calling function
		return $post_types;
